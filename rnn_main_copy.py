...
def evaluate_(model, validiter, criterion, device):
    ...
    for xs, ys, end_of_data in validiter:
        ...
        loss = criterion(outs, ys.type(torch.int64).squeeze())

        total_loss += loss.item()
        n_segs += 1

        _, output_index = torch.max(outs,1)
        output_index = output_index.reshape(-1,1).detach().cpu().numpy()
        ys = ys.cpu().numpy()
    ...
    return {'loss': total_loss / n_segs, 'acc': acc, 'prec': prec, 'rec': rec, 'f1': f1}
'''
def evaluate_(model, validiter, criterion, device):
    model.eval()
    total_loss = 0
...
def train(
    ...
    if args.optimizer == 'Adam':
        ...
        optimizer = eval(mystring)(net.parameters(), lr=args.lr, momentum = args.momentum, weight_decay = args.weight_decay)
    ...
    for epoch in range(num_epochs):
        ...
        val_result = evaluate_(net, valid_loader, criterion, device)
        print('epoch: {:d} | train: {:.4f} | val loss: {:.4f}, f1: {:.4f}, prec: {:.4f}, rec: {:.4f}, acc {:.4f}'.format(epoch+1, train_loss, val_result['loss'], val_result['f1'], val_result['prec'],val_result['rec'], val_result['acc']))
        #neptune.log_metric('tr loss', epoch, train_loss)
        #neptune.log_metric('val loss', epoch, val)
        f1 = val_result['f1']
        if epoch==0 or best_val < f1:
